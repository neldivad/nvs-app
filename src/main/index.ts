...
protocol.registerSchemesAsPrivileged([
  // supportFetchAPI: DialogZoomCrop loads assets via fetch -> blob URL (an
  // <img> from a custom scheme can taint the export canvas). Without it,
  // renderer fetch() rejects the scheme outright and the crop stays blank.
  { scheme: 'nvs-asset', privileges: { bypassCSP: true, stream: true, supportFetchAPI: true } },
...
function registerIpc(): void {
  ...
  ipcMain.handle(CHANNELS.checkForUpdate, () => checkForUpdate())
  ...
  // Generated bytes (a zoom-crop result): same asset dir + naming discipline as
  // importImageBatch, but there is no source file to read. PNG as-is: crops are
  // small, may carry alpha, and re-encoding to JPEG here would fill it black.
  ipcMain.handle(
    CHANNELS.importImageBytes,
    async (_event, pageId: string, _kind: string, baseName: string, bytes: ArrayBuffer) => {
      const project = engine.currentProject()
      if (!project) return null
      const safe = String(baseName).replace(/[^a-z0-9-]/gi, '').slice(0, 24) || 'crop'
      const dir = `content/assets/${pageId || 'media'}`
      mkdirSync(join(project.root, dir), { recursive: true })
      const filename = `${Date.now()}-${safe}.png`
      writeFileSync(join(project.root, dir, filename), Buffer.from(bytes))
      return `${dir}/${filename}`
    }
  )
  ...
  ipcMain.handle(CHANNELS.startCoherenceRun, (_e, opts?: { critique?: boolean }) => startCoherenceRun(opts))
