...
export function PermissionSummary({ required, optional, cap = 3 }: { required: string[]; optional?: string[]; cap?: number }): JSX.Element {
  ...
  if (all.length === 0) {
    return (
      <span className="flex items-center gap-0.5 text-ok">
        <ShieldCheck className="size-3 shrink-0" /> no access needed
      </span>
    )
  }
  ...
  return (
    <>
      ...
      {all.length > cap && <span className="text-faint">+{all.length - cap} more</span>}
    </>
  )
}
  ...
  if (required.length === 0 && (optional?.length ?? 0) === 0) {
    return (
      <div className="mt-4 flex items-center gap-2 rounded-lg border border-ok/30 bg-ok/5 px-3 py-2 type-caption text-ok">
        <ShieldCheck className="size-3.5 shrink-0" /> No access needed — this extension can't read or change anything in your work.
      </div>
    )
  }
      ...
      <div key={id} className={cn('flex items-start gap-2.5 px-3 py-2', dim && 'opacity-60')}>
        ...
        <div className="min-w-0">
          <div className="type-caption font-medium text-foreground">
            {g.label}
            {dim && <span className="type-micro ml-1 text-faint">(optional)</span>}
          </div>
  ...
  return (
    <div className="mt-4">
      <div className="mb-1.5 flex items-center gap-1 type-label text-faint">
        <ShieldCheck className="size-3" /> What this extension can access
      </div>
      ...
      <p className="mt-1.5 type-micro text-faint">Extensions run in a separate process and get only the access listed here — nothing else on your machine.</p>
    </div>
  )
}
