...
export function BookCard({ info: rawInfo, fallbackTitle, onEdit, basePath, preview }: { info: ProjectInfo; fallbackTitle: string; onEdit?: () => void; basePath?: string; preview?: boolean }): JSX.Element {
  ...
  add('Kind', info.domain ? <KindChip domain={info.domain} /> : null, !!info.domain)
  add('Type', info.medium ? labelOf(MEDIUMS, info.medium) : null, !!info.medium)
  add('Status', info.status ? labelOf(STATUSES, info.status) : null, !!info.status)
  add('Author', info.author, !!info.author)
  add('Language', langs.length ? langs.join('   ') : null, langs.length > 0)
  add(info.domain === 'nonfiction' ? 'Subject' : 'Genre', info.genre?.length ? <Chips key="g" opts={GENRES} vals={info.genre} tone="text-character" /> : null, !!info.genre?.length)
  add('Themes', info.about?.length ? <Chips key="t" opts={THEMES} vals={info.about} tone="text-lore" /> : null, !!info.about?.length)
  add('Warnings', info.contentRating?.length ? <Chips key="w" opts={CONTENT_WARNINGS} vals={info.contentRating} tone="text-flag" /> : null, !!info.contentRating?.length)
  add('Keywords', info.keywords?.length ? info.keywords.join(', ') : null, !!info.keywords?.length)
  ...
  if (info.isPartOf) relations.push(['Series', info.position ? `${info.isPartOf} · #${info.position}` : info.isPartOf])
  if (info.isBasedOn) relations.push(['Based on', info.isBasedOn])
  if (info.forkedFrom) relations.push(['Forked from', info.forkedFrom])
  if (info.coverOf) relations.push(['Cover of', info.coverOf])
  if (info.promptSource) relations.push(['Prompt', info.promptSource])
      ...
      {(info.description || preview) && (
        <Section title="Synopsis">
          {info.description ? (
            ...
            <p className="text-[12px] italic text-faint">No synopsis yet.</p>
          )}
        </Section>
      )}

      {credits.length > 0 && (
        <Section title={`Credits${credits.length > 1 ? ` · ${credits.length}` : ''}`}>
          <div className="overflow-hidden rounded-md border border-border/60">
            {shownCredits.map((c, i) => (
              <div key={i} className={cn('grid grid-cols-[1fr_1.3fr] items-baseline gap-2 px-2.5 py-1 text-[12px]', i % 2 === 1 && 'bg-panel-soft/30')}>
                ...
                <span className="wrap-break-word text-faint">{c.roles.map((r) => labelOf(CONTRIBUTOR_ROLES, r)).join(', ') || 'contributor'}</span>
              </div>
            ))}
          </div>
          {credits.length > 10 && (
            <button onClick={() => setShowAllCredits((v) => !v)} className="mt-1 text-[11px] text-thread hover:underline">
              {showAllCredits ? 'Show less' : `Show all ${credits.length}`}
            </button>
          )}
        </Section>
      )}
