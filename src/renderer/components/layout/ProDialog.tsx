...
const SHARE_TEXT = 'Writing with Novel Visual Studio. It is a writing IDE for dialogue-driven fiction, with optional AI reading features.'
...
export function ProDialog({ open, onClose }: { open: boolean; onClose: () => void }): JSX.Element {
  ...
  const shareX = (): void =>
    openUrl(`https://x.com/intent/tweet?text=${encodeURIComponent(SHARE_TEXT)}&url=${encodeURIComponent(NVS_URL)}`)
  ...
  const activate = async (): Promise<void> => {
    ...
    try {
      ...
      setEnt(e)
      await refreshPro()
      setMsg(e.pro ? 'Pro activated ✓' : 'No Pro purchase found for that email.')
    } catch {
      setMsg('Couldn’t verify right now — try again.')
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <Dialog open={open} onClose={onClose} title="NVS Pro" size="panel" accent="#f59e0b" region="proDialog">
      <div className="space-y-5">
        {pro ? (
          ...
          <div className="flex flex-col items-center gap-2 py-2 text-center">
            ...
            <div className="type-card-title text-foreground">Pro is active</div>
            {ent?.email && (
              <div className="type-caption text-muted-foreground">
                Tied to <span className="text-foreground">{ent.email}</span>
              </div>
            )}
            <p className="type-caption max-w-xs text-faint">
              Thank you for supporting NVS. Unlocks the prettier chart-export theme today; more Pro niceties are on
              the way. Your writing and files are always free &amp; local.
            </p>
          </div>
        ) : (
          // ── Coming soon + share ──────────────────────────────────────────────
          <>
            <div className="flex flex-col items-center gap-2 py-1 text-center">
              ...
              <div className="type-card-title text-foreground">Pro features are coming soon</div>
              <p className="type-body-sm max-w-sm text-muted-foreground">
                Writing, analysis, and your files are free forever. Pro will add a few optional niceties down the line (maybe prettier exports).
              </p>
            </div>
            ...
            <div className="rounded-lg border border-border bg-panel-soft/40 p-4">
              <div className="mb-3 flex items-center justify-center gap-1.5 text-center">
                ...
                <span className="type-label text-foreground">In the meantime, show your support by sharing</span>
              </div>
              <div className="flex gap-2">
                ...
                >
                  <Share2 className="size-3.5" /> Share on X
                </button>
                <button
              ...
              <div className="mt-3 flex items-center justify-center gap-4">
                ...
                <button onClick={() => openUrl(qedConfig.links.discord)} className="flex items-center gap-1.5 type-caption text-muted-foreground hover:text-foreground">
                  <MessagesSquare className="size-3.5" /> Join Discord
                </button>
              </div>
            </div>
          </>
          ...
          <div className="border-t border-border pt-3 text-center">
            {showActivate ? (
              <div className="mx-auto max-w-xs space-y-1.5">
                <div className="type-caption text-faint">Already have a license? Enter your purchase email:</div>
