...
export const FindBar = forwardRef<HTMLInputElement, FindBarProps>(function FindBar(
  { query, count, active, onQueryChange, onNext, onPrev, onClose, placeholder = 'Find' },
  ref
): JSX.Element {
  return (
    <div className="absolute right-3 top-3 z-20 flex items-center gap-1 rounded-md border border-border bg-panel px-2 py-1 shadow-lg">
      ...
      <button onClick={onPrev} disabled={!count} title="Previous (⇧⏎)" className="rounded p-0.5 text-muted-foreground hover:bg-panel-soft disabled:opacity-40"><ChevronUp className="size-3.5" /></button>
      <button onClick={onNext} disabled={!count} title="Next (⏎)" className="rounded p-0.5 text-muted-foreground hover:bg-panel-soft disabled:opacity-40"><ChevronDown className="size-3.5" /></button>
      <button onClick={onClose} title="Close (Esc)" className="rounded p-0.5 text-muted-foreground hover:bg-panel-soft"><X className="size-3.5" /></button>
    </div>
  )
})
